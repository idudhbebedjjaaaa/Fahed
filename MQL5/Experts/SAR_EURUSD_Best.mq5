//+------------------------------------------------------------------+
//| SAR_EURUSD_Best.mq5                                             |
//| Stop-And-Reverse EA - Optimized for EURUSD                      |
//| Standalone file - no external includes needed                    |
//|                                                                  |
//| Timeframe: M15 | HTF Filter: H1                                 |
//| Session: London+NY Overlap (13:00-17:00 UTC)                     |
//|                                                                  |
//| Real Data Backtest Results (Jan 2026 - May 2026):                |
//|   Net Profit: $4,424 | Trades: 58 | Win Rate: 39.7%             |
//|   Profit Factor: 3.13 | Sharpe: 2.65 | Max DD: 3.3%             |
...
#property description "SAR EURUSD - Optimized on Real Data"
#property strict
...
input bool   InpVolAdaptive     = false;         // Volatility Adaptive Distance
...
int OnInit()
{
   if(StringFind(_Symbol, "EUR") < 0)
      Print("WARNING: EA optimized for EURUSD. Current: ", _Symbol);
   ...
   Print("=== SAR EURUSD v3.0 (Real Data Optimized) ===");
   Print("Distance: ATR x", InpATRDistMult, " | Trail: VOL x", InpTrailMult);
...
void OnDeinit(const int reason)
{
   ...
   Print("SAR EURUSD stopped. Reversals: ", g_totalReversals);
}
...
bool OpenTrade(int dir, double atrVal)
{
   ...
   if(dir > 0)
   {
      ...
      result = g_trade.Buy(lots, _Symbol, price, 0, 0, "SAR_EUR");
   }
   else
   {
      price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      result = g_trade.Sell(lots, _Symbol, price, 0, 0, "SAR_EUR");
   }
...
void PlacePendingOrder(double atrVal)
{
   ...
   if(g_direction > 0)
   {
      ...
      if(g_trade.SellStop(lots, price, _Symbol, 0, 0, ORDER_TIME_GTC, 0, "SAR_EUR_REV"))
   ...
   else
   {
      ...
      price = NormalizeDouble(price, _Digits);
      if(g_trade.BuyStop(lots, price, _Symbol, 0, 0, ORDER_TIME_GTC, 0, "SAR_EUR_REV"))
         g_pendTicket = g_trade.ResultOrder();
   }
